...
export default function Edit( props ) {
    ...
    const { ariaExpanded } = attributes;


    const template = [
        [ 'core/paragraph', { placeholder: 'Create Expanded State...' } ],
    ];
    ...
    const blockProps = useBlockProps();
        ...
        return (
            ...
            <InspectorControls>
				<PanelBody>
					<TextControl
						...
						value={ ariaExpanded }
						onChange={ ( ariaExpanded ) => setAttributes( { ariaExpanded } ) }
						help={__(
			...
			<span className="screen-reader">{ariaExpanded}</span>
            <InnerBlocks
                template = { template }
				templateLock = { false }
            />
