    ...
    const taxonomySelect = (
        <>
            ...
            {includePastEvents && (
                <>
                    <ToggleControl
                        label={__("Only show past events")}
                        checked={onlyPastEvents}
                        help={__("Limit the event to only show past events.")}
                        onChange={(value) => setAttributes({ onlyPastEvents: value })}
                    />
                    <SelectControl
                        label={__("Sort Order", "carkeek-blocks")}
                        help={__("Events are always sorted by start date, select ASC or DESC.")}
